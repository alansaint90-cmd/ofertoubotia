...
import { EvolutionConnection } from "@/components/evolution-connection";
...
export function App() {
  ...
  const [queuedStatus, setQueuedStatus] = useState("");
  ...
  useEffect(() => {
    if (!queuedRequestId || !["queued", "processing"].includes(queuedStatus)) return;
    const timer = window.setInterval(async () => {
      try {
        const response = await fetch(`/api/dispatches?requestId=${encodeURIComponent(queuedRequestId)}`, { cache: "no-store" });
        if (response.ok) { const result = await response.json() as { status: string }; setQueuedStatus(result.status); }
      } catch { /* A consulta pode ser repetida no próximo intervalo. */ }
    }, 5000);
    return () => window.clearInterval(timer);
  }, [queuedRequestId, queuedStatus]);
  ...
  const startOffer = (product: Product) => {
    ...
    setOfferConfirmed(false); setQueuedRequestId(""); setQueuedStatus("");
  ...
  const publishOffer = async () => {
    ...
    try {
      ...
      setNotice("Oferta colocada na fila para o único grupo autorizado. Acompanhe o estado abaixo.");
    } catch { setNotice("Falha de rede ao colocar a oferta na fila."); }
